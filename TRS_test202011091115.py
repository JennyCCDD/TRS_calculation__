...
@revise log:
    ...
    2020.11.06 测试并修改FIFO
               ...
               修复当天交易所有持仓股票的情况
               单独处理正好完全清仓的情况
    2020.11.09 修复持仓数量，持仓数量=累计成交数量+累计股票股利
                补充代码注释
"""
# In[]
import pandas as pd
...
def todayPerform(Date,Position,Today,Type):
    ...
    if Type == 'futures':
        # 对期货的证券代码进行处理否则wind取不到结算价，Position['证券代码']为昨日持仓股票列表，asset_not_trade为未交易股票列表
    ...
    # 如果存在未交易昨日已有持仓的股票，需要将今日交易部分与之合并，交易数量和交易价格等列为nan
    if len(asset_not_trade) != 0:
    ...
    # 对position dataframe的index进行重排序否则报错
    ...
    Position.loc[:, '持仓数量'] = np.array(pd.DataFrame(Position.groupby(['证券代码'])['交易数量'].cumsum())['交易数量']) + \
                              np.array(pd.DataFrame(Position.groupby(['证券代码'])['股票股利'].cumsum())['股票股利'])

    # 对卖出进行标记，剩余数量为该笔买入对应的持仓未卖出的部分（FIFO计算需要）
    ...
    # 对于卖出交易，用先进先出法计算持仓成本，并调整对应买入交易的剩余数量
    for stocki in Position_sell.to_list():
        Position.loc[Position['证券代码']== stocki,:] = \
        FIFO(Position.loc[(Position['证券代码']== stocki)])
...
if __name__ == '__main__':
    ...
    if os.path.exists('./data/新综合信息查询_成交回报明细（股票）.xls') == True:
        ...
        today = pd.DataFrame(columns= ['证券代码','交易数量', '交易价格','交易费用','印花税',
                                       '收盘价','成本价格','合约乘数'])
    ...
    if os.path.exists('./data/新综合信息查询_成交回报明细（股指）.xls') == True:
        ...
        tdate = input('请输入日期（格式为xxxx-xx-xx）:')
        today_futures = pd.DataFrame(columns= ['证券代码','交易数量', '交易价格','交易费用','印花税',
                                       '收盘价','成本价格','合约乘数'])
